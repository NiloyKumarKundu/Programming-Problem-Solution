...
bool linearDiophantine(int A, int B, int C, int *x, int *y) {
	...
	extendedEuclidean(a, b, x, y);	 // Solve ax + by = 1

	if (g < 0) {   // Make sure gcd(a, b) = 1 OR gcd is always positive
		a *= -1, b *= -1, c *= -1;
	}
	*x *= c, *y *= c;	// ax + by = c;
...
void solve() {
	int a, b, c, x, y;
	cin >> a >> b >> c;
	int res = linearDiophantine(a, b, c, &x, &y);
    if (!res) {
		cout << "No solution\n";
		return;
	}
    cout << "One possible solution (" << x << ", " << y << ")\n";
    int g = gcd(a, b);
    int k = 1;
	cout << "Another possible solution (" << x + k * (b / g) << ", " << y - k * (a / g) << ")\n";
	k = 2;
	cout << "Another possible solution (" << x + k * (b / g) << ", " << y - k * (a / g) << ")\n";
	// we can use k = -infinity to +infinity.
}
